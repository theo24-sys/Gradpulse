...
def guest_login(request, portal):
    """
    Log in automatically using a shared dummy guest account for the requested portal.
    Creates the dummy account with mock data if it does not exist.
    """
    if portal == 'employer':
        username = 'guest_employer'
        defaults = {
            'portal_type': 'employer',
            'first_name': 'Guest',
            'last_name': 'Employer',
            'company_name': 'TechPulse Innovations (Mock)',
            'company_about': 'A leading technology solutions provider specializing in AI and web development.',
            'sector': 'Technology',
            'website': 'https://example.com',
            'is_verified': True,
        }
    elif portal == 'unismart':
        username = 'guest_unismart'
        defaults = {
            'portal_type': 'unismart',
            'first_name': 'Guest',
            'last_name': 'UniSmart',
            'student_category': 'sss',
            'grade_level': 'Form 4',
        ...
        defaults = {
            ...
            'bio': 'A passionate software engineering student looking for internship opportunities.',
            'is_verified': True,
        }
    ...
    # If the user already existed but someone emptied their demo data, you could optionally refresh it here.
    
    login(request, user, backend='django.contrib.auth.backends.ModelBackend')
    messages.success(request, f"You are now exploring as a Guest {portal.capitalize()}!")
