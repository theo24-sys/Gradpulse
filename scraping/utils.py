...
def get_items_for_student(student_profile=None, source_type=None, limit=20):
    ...
    # Only show approved (or pending for now if you want, but prompt says "approved")
    # Actually, the prompt says "all approved ScrapedItems" in fallback.
    # I'll use status='approved' for production-like behavior, 
    # but I'll include 'pending' for initial dev testing if it's empty.
    approved_query = base_query.filter(status='approved')
    
    # If no approved items exist, use all (to avoid empty page during dev)
    if not approved_query.exists():
        approved_query = base_query.exclude(status='rejected')

    if not student_profile:
        return approved_query.order_by('-scraped_at')[:limit]
    ...
    matched_items = approved_query.filter(filters).order_by('-scraped_at')
    
    # Fallback logic: if fewer than 10 matched items, fill with general items
    if matched_items.count() < 10:
        general_items = approved_query.exclude(id__in=matched_items.values_list('id', flat=True)).order_by('-scraped_at')
